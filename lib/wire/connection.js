...
var Connection = function (host, port, callback, options, db, svc) {
    ...
    options.charsetHoosk = options.charsetHoosk || {};
    options.charsetHoosk.text = options.charsetHoosk.text || {};
...
Connection.prototype._bind_events = function(host, port, callback) {
    ...
    self._socket.on('data', function (data) {
        ...
        if (!self._xdr) {
            ...
            delete (self._xdr); // @todo replace with self._xdr = undefined;
        }
        ...
        while (xdr.pos < xdr.buffer.length) {
            ...
            decodeResponse(xdr, cb, self, function (err, obj) {
                ...
                if (err) {
                    /**
                     * @todo - this is a silent error. Maybe we should be able to emit these type of errors?
                     *         i.g. Reading errors -> RangeError [ERR_OUT_OF_RANGE]: The value of "offset" is out of range. It must be >= 0 and <= 356. Received 360
                     */
                    console.error('decodeResponse failed with error: ', err);
...
Connection.prototype.createDatabase = function (options, callback) {
    ...
    blr.pos = 0;
    blr.addByte(Const.isc_dpb_version1);
    blr.addString(Const.isc_dpb_set_db_charset, options.encoding || 'UTF8', this._charset); // set's the new DB default CHARSET
    blr.addString(Const.isc_dpb_lc_ctype, options.encoding || 'UTF8', this._charset); // set's the connection charset
    blr.addString(Const.isc_dpb_user_name, user, this._charset);
    if (this.accept.protocolVersion < Const.PROTOCOL_VERSION13) {
        if (this.accept.protocolVersion === Const.PROTOCOL_VERSION10) {
            blr.addString(Const.isc_dpb_password, password, this._charset);
