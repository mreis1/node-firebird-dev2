...
/**
 * Returns the amount of bytes required to fit n bytes of data in a 4 bytes block
 * i.g. align(5) outputs 8
 *      align(4) outputs 4
 *      ...
 *
 * @param n
...
/**
 * Determines the length by reading the int value in buffer at current position.
 * Increments the position and slices the existing buffer current position for the pre-determined length
 *
 * @returns {*}
 */
...
XdrReader.prototype.readText = function(len, optsOrCharset) {
    ...
    opts.charsetHooks.text = opts.charsetHooks.text || {};
